...
from pcd_downsampling import grid_sample_pcd, random_uniform_downsample, color_weighted_downsample
...
class SingleVisionProcess(Process):
    def __init__(
        ...
        # whether to use color-weighted uniform random downsampling
        self.use_color_weighted_downsample = use_color_weighted_downsample
    ...
    def run(self):
        ...
        while True:
            ...
            if self.enable_pointcloud and point_cloud_frame is not None:
                ...
                # 基于颜色的随机均匀采样（可选）
                if self.use_color_weighted_downsample:
                    point_cloud_frame = color_weighted_downsample(point_cloud_frame, self.num_points, target_color=(255, 255, 0), temperature=30.0)
                else:
    ...
    def __init__(
        ...
        if use_front_cam:
            self.front_process = SingleVisionProcess(
                ...
                use_color_weighted_downsample=use_color_weighted_downsample,
                img_size=img_size,
            )
        if use_right_cam:
